...
download_date <- "2024-05-27"
out_prefix <- "2024_05_27"
date_w_space <- "20240527"
...
#lineage.interest.v <- c("HK.3","BA.2.86.1","JN.1","KP.2","KP.3","LB.1","KP.2.3")
metadata.filtered.t <- metadata.filtered %>% filter(Pango.lineage %in% lineage.interest.v)
head(metadata.filtered)

#nrow(metadata.filtered.2)
#nrow(filter(metadata.filtered,Collection.date >= date.start, Collection.date <= date.end))
...
count.pango.df.kp2 <- count.pango.df %>% filter(Pango.lineage=="KP.2") %>% arrange(desc(count)) ##### Count of KP.3 in each country
count.pango.df.kp3 <- count.pango.df %>% filter(Pango.lineage=="KP.3") %>% arrange(desc(count)) ##### Count of KP.3 in each country
count.pango.df.lb1 <- count.pango.df %>% filter(Pango.lineage=="LB.1") %>% arrange(desc(count)) ##### Count of KP.3 in each country
count.pango.df.kp23 <- count.pango.df %>% filter(Pango.lineage=="KP.2.3") %>% arrange(desc(count)) ##### Count of KP.3 in each country
count.pango.df.kp32 <- count.pango.df %>% filter(Pango.lineage=="KP.3.2") %>% arrange(desc(count)) ##### Count of KP.3 in each country
...
count.pango.df.Australia <- count.pango.df %>% filter(country=="Australia", count >= 20) %>% arrange(desc(count))
count.pango.df.Singapore <- count.pango.df %>% filter(country=="Singapore", count >= 20) %>% arrange(desc(count))
...
g1 <- g1 + scale_y_continuous(limits=c(0.8,1.6),breaks=c(0.8,0.9,1,1.1,1.2,1.3,1.4,1.5,1.6))
g1
